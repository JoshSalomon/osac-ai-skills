# OSAC AI skills evals

Skill-quality evaluation tooling for measuring agentic SDLC quality. Lives in
`osac-ai-skills` alongside `skills/`. Case inputs for review evals still expect
sibling checkouts of `enhancement-proposals/` and a `.design/` context (as used
by `osac-workspace` / future `osac/` bootstrap consumers) — this repo does not
vendor those siblings.
...
Run all eval commands from the **`osac-ai-skills` root** (the directory that
contains `skills/` and `evals/`). For review cases that reference documents under
`enhancement-proposals/`, ensure that sibling (and `.design/`) is available via
the symlink/workspace layout your consumer bootstrap provides.

1. **Consumer bootstrap** — From a checkout that vendors this repo (today
   `osac-workspace`; later `osac/` per ADR 0001 Decision item 2), run that
   repo's bootstrap so agent skills are linked and `enhancement-proposals` is
   available as a sibling when review cases need it.
...
3. **agent-eval-harness** — pinned checkout bootstrapped locally:
   ...
   Harness is **not** installed by consumer bootstrap — run `setup-harness.sh`
   when you need review evals.
